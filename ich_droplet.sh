...
sudo install.r shiny 

# export SHINY_VERSION=$(wget --no-check-certificate -qO- https://s3.amazonaws.com/rstudio-server/current.ver)
export SHINY_VERSION=1.5.3.838
wget https://download3.rstudio.org/ubuntu-12.04/x86_64/shiny-server-${SHINY_VERSION}-amd64.deb
sudo gdebi --non-interactive shiny-server-${SHINY_VERSION}-amd64.deb
sudo rm shiny-server-*-amd64.deb 
